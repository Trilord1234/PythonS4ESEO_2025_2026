...
func _on_generation_pressed():
	"""
	Instancie et place aléatoirement une nouvelle planète dans l'univers.

	Incrémente le compteur total affiché à l'écran, génère une nouvelle instance 
	de planète, et lui attribue une position initiale aléatoire strictement comprise 
	dans les limites de la zone spatiale (HitBox_Box). Connecte également le signal 
	de sélection pour permettre l'interaction du joueur.
...
func _on_clear_pressed():
	"""
	Réinitialise complètement l'affichage de l'univers et la mémoire du backend.

	Remet le compteur visuel à zéro, parcourt et détruit (queue_free) toutes 
	les instances de planètes présentes à l'écran, puis déclenche l'effacement 
	des données du graphe et des planètes côté serveur Python via GlobalData.
